...
const buttonVariants = cva(
	"inline-flex select-none items-center justify-center rounded-md font-semibold transition-transform transition-colors duration-200 hover:cursor-pointer focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring disabled:cursor-not-allowed disabled:opacity-60 active:scale-[0.98] group hover:bg-white/85",
	{
		variants: {
			variant: {
				primary: "bg-white text-primary-foreground disabled:bg-white/30",
				secondary: "border border-secondary text-white hover:bg-secondary",
				muted:
		...
		return (
			...
			>
				{isLoading ? (
					...
					<>
						{leftIcon ? (
							<span className="mr-1 -ml-0.5 inline-flex">{leftIcon}</span>
						) : null}
						{typeof children === "string" ? (
							<span
								className={cn(
									"relative inline-flex overflow-hidden uppercase",
